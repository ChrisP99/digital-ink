...
The web app is firstly cloned from its repository via the \mintinline{git}|git clone| command, and a new
...
\begin{enumerate}
    ...
    \item \item \mintinline{docker}|phpmyadmin|: A way to locally manage the database through a UI\@.
\end{enumerate}

At the minute, the web app is live through the \mintinline{docker}|digital-ink|and is using a local version of MySQL
as a database, stored within the \mintinline{docker}|mysql|Docker container.
At present, the database has no tables, but can be
